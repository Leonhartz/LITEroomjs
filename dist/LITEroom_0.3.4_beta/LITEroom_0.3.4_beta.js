...
var LITEroom_Version = "0.3.4 beta";
...
AFRAME.registerSystem('lite_fly_here',{
    ...
    fly: function(comp){
        ...
        if(this.current_job != null && !this.current_job.data.allow_interrupt) return;
        
        var self = this;
        this.camera = $_("a-camera") || $_("a-entity[camera]");
        
        if(this.camera === undefined || this.camera === null){
            console.error("lite_fly_here system: Cannot find the camera!")
            return;
        }
...
AFRAME.registerComponent('lite.input-scalebar',{
  ...
  init: function(){
    ...
    function updateFg(){
      var length = (self.data.value - self.data.min) / (self.data.max - self.data.min) * self.data.length;
    ...
    if(self.data.layout == "vertical"){
      if(self.data.anchor == "r" || self.data.anchor == "right"){
        pos_p = "0 " + (-this.data.length / 2 - 0.5) + " 0";
        pos_s = "0 " + (this.data.length / 2 + 0.5) + " 0";
      }else{
        pos_p = "0 " + (this.data.length / 2 + 0.5) + " 0";
        pos_s = "0 " + (- this.data.length / 2 - 0.5) + " 0";
      }
      ...
      if(self.data.anchor == "r" || self.data.anchor == "right"){
        pos_s = (this.data.length / 2 + 0.5) + " 0 0";
        pos_p = (- this.data.length / 2 - 0.5) + " 0 0";
      }else{
        pos_p = (this.data.length / 2 + 0.5) + " 0 0";
        pos_s = (- this.data.length / 2 - 0.5) + " 0 0";
      }
