...
*  ========= 0.3.5 ========== (21/Aug/2018)
*  ========= 0.38r1 ========== (25/Sep/2018)
*  - Added 'selected' status making it work with buttons manager
...
AFRAME.registerComponent('lite_button',{
    ...
    init: function(){
        ...
        var begin = new CustomEvent("begin", {bubbles: false});
        var begin2 = new CustomEvent("begin2", {bubbles: false});
        var end = new CustomEvent("end", {bubbles: false});
        var end2 = new CustomEvent("end2", {bubbles: false});
        
        var anim1 = this.anim1 = document.createElement("a-animation");
        anim1.setAttribute("attribute", "scale");
        anim1.setAttribute("dur", 400);
        anim1.setAttribute("from", "1 1 1");
        anim1.setAttribute("to", "1.1 1.1 1.1");
        anim1.setAttribute("easing", "ease-out-elastic");
        anim1.setAttribute("begin", "begin");
        anim1.setAttribute("end", "end");
        container.appendChild(anim1);
        
        
        var anim2 = this.anim2 = document.createElement("a-animation");
        anim2.setAttribute("attribute", "scale");
        anim2.setAttribute("dur", 200);
        anim2.setAttribute("to", "1 1 1");
        anim2.setAttribute("begin", "begin2");
        anim2.setAttribute("end", "end2");
        container.appendChild(anim2);
        ...
        bg.addEventListener("mouseenter", function (){
            ...
            if(self.data.active){
                ...
                self.container.dispatchEvent(end2);
                self.container.dispatchEvent(begin);
            }
        ...
        bg.addEventListener("mouseleave", function (){
            ...
            self.container.dispatchEvent(end);
            self.container.dispatchEvent(begin2);
        });
...
AFRAME.registerComponent('lite_round_button',{
    ...
    update: function(oldData){
        ...
        bg.setAttribute("geometry", {radius: Math.max(data.radius,0)});
        ...
        fg.setAttribute("geometry", {radius: Math.max(data.radius - data.margin,0)});
        fg.setAttribute("position", "0 0 0.005");
        
        var label = this.label;
        if (data.labelType == "image") {
            label.setAttribute("material", {src: data.label, transparent: true});
            label.setAttribute("geometry", {radius: Math.max((data.radius - data.margin)*data.labelSize,0)});
        } else {
...
AFRAME.registerComponent('lite.videoplayer',{
  schema:{
    ...
    playBtn_src: {default: "/dev/shared/icons/icons8-circled-play-80.png"}, // texture file for play button
    pauseBtn_src: {default: "/dev/shared/icons/icons8-pause-button-80.png"}, // texture file for pause button
